...
export const DeleteConfirmModal: React.FC<DeleteConfirmModalProps> = ({
  ...
  const displayTitle = title || (packageItem ? `Delete "${packageItem.name}"?` : 'Confirm Deletion');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs">
      <div className="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-6 overflow-hidden">
        ...
        <button
          ...
          disabled={isDeleting}
          className="absolute top-4 right-4 p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 disabled:opacity-50 transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="mt-6 flex items-center justify-end space-x-3">
          <button
            ...
            onClick={handleDismiss}
            disabled={isDeleting}
            className="px-4 py-2 text-xs font-semibold rounded-lg text-slate-300 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 transition-colors cursor-pointer"
          >
            Cancel
          </button>
          <button
            ...
            onClick={onConfirm}
            disabled={isDeleting}
            className="inline-flex items-center px-4 py-2 text-xs font-semibold rounded-lg text-white bg-rose-600 hover:bg-rose-500 focus:outline-hidden focus:ring-2 focus:ring-rose-500 disabled:opacity-50 transition-colors cursor-pointer"
          >
            {isDeleting ? (
              <>
